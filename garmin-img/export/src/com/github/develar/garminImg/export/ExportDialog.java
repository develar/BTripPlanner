 ...
 */
package com.github.develar.garminImg.export;

import org.free.garminimg.ImgFilesBag;
import org.free.garminimg.swing.MapControlPanel;
...
public class ExportDialog<COORD> extends JDialog implements FileExporterConfigListener {
  ...
  public ExportDialog(Frame owner, ImgFilesBag map, MapTransformer transformer) {
    ...
    selectionMapPanel = new ExportMapPanel<>(map, selectionTransformer);
    selectionControlPanel = new MapControlPanel<>(selectionMapPanel);
  ...
  private void generateFile() {
    ...
    try {
      fileExporter.setup(file, generateTransformer.getWidth(), generateTransformer.getHeight());
      selectionMapPanel.saveMapAs(fileExporter.getG2(), generateTransformer, fileExporter.getFontSize());
      fileExporter.finishSave();
    }
    catch (IOException e) {
      e.printStackTrace();
    }
  ...
  private JPanel createQualityPanel(ImgFilesBag map) {
    ...
    mapSamplePanel = new ExportMapPanel<>(map, sampleTransformer);
    //c.gridy++;
    c.weighty = 1;
    c.fill = GridBagConstraints.BOTH;
  ...
  private void updateSelectionPanelSize() {
    ...
    if (fillerWidth <= 0 || fillerHeight <= 0) {
      return;  //weird swing behavior...
    }

    double ratio = (double)height / width;
    double fillerRatio = (double)fillerHeight / fillerWidth;
    if (fillerRatio >= ratio) {   //height bigger
