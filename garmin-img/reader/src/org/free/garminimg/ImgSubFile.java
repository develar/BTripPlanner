...
public abstract class ImgSubFile {
  ...
  public static ImgSubFile create(String filename, String filetype, int fileSize, int blocSize, int initialOffset, ImgFileBag fileBag) {
    if (filetype.equalsIgnoreCase("LBL")) {
      return new LblSubFile(filename, filetype, fileSize, blocSize, initialOffset, fileBag);
    }
    else if (filetype.equalsIgnoreCase("TRE")) {
      return new TreSubFile(filename, filetype, fileSize, blocSize, initialOffset, fileBag);
    }
    else if (filetype.equalsIgnoreCase("RGN")) {
      return new RgnSubFile(filename, filetype, fileSize, blocSize, initialOffset, fileBag);
    }
    else if (filetype.equalsIgnoreCase("NET")) {
      return new NetSubFile(filename, filetype, fileSize, blocSize, initialOffset, fileBag);
    }
    else {
      return new UnknownSubFile(filename, filetype, fileSize, blocSize, initialOffset, fileBag);
    }
  }

  public ImgSubFile(String filename, String filetype, int fileSize, int blocSize, int initialOffset, ImgFileBag fileBag) {
  ...
  public void seek(long pos, FileContext context) throws IOException {
    ...
    if (context.curBloc != newBloc) {
      ...
      context.curBlocContent = fileBag.getBloc((firstBloc + newBloc) * blocSize, blocSize);
    }
