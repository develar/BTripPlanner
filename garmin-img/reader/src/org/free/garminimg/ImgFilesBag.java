...
public class ImgFilesBag {
  ...
  private int maxLat = Integer.MIN_VALUE, maxLon = Integer.MIN_VALUE, minLat = Integer.MAX_VALUE, minLon = Integer.MAX_VALUE;


  /**
   * Add a single .img file to the repository.
   */
  public synchronized void addFile(File file, boolean fullInit, SubFileReader subFileReader) throws IOException {
    ImgFileBagExtractor extractor = new ImgFileBagExtractor(file, this, subFileReader);
   ...
   */
  public synchronized void readMapForDrawing(int minLong, int maxLong, int minLat, int maxLat, int resolution, int objectKindFilter, MapListener listener) throws IOException {
    if (!mapFiles.isEmpty()) {
   ...
   */
  public synchronized void readMap(int minLong, int maxLong, int minLat, int maxLat, int resolution, int objectKindFilter, BitSet objectTypeFilter, MapListener listener)
    throws IOException {
    ...
    try {
      if (mapFiles.size() > 0) {
        for (ImgFileBag img : mapFiles) {
          ...
          int num1 = (getMaxLongitude() - getMinLongitude()) / (e - w);
  ...
  static synchronized void registerOpenFile(ImgFileBag bag) throws IOException {
    ...
    while (openFiles.size() >= MAX_OPEN_FILES && cpt < openFiles.size()) {
      ...
      if (!toClose.close()) {   //failed (was in use), retry with another one
  ...
  private void readMap(int minLong,
                       int maxLong,
                       int minLat,
                       int maxLat,
                       int resolution,
                       ...
                       SortedSet<ImgFileBag> files) throws IOException {
  ...
  private void readMapForDrawing(int minLong, int maxLong, int minLat, int maxLat, int resolution, MapListener listener, SortedSet<ImgFileBag> files, int objectKindFilter)
    throws IOException {
    ...
    if ((objectKindFilter & ObjectKind.EXTENDED_POLYGON) != 0) {
      readMap(minLong, maxLong, minLat, maxLat, resolution, ObjectKind.EXTENDED_POLYGON, null, listener, files);
    }

    //lines and points can be read in any order.
    if ((objectKindFilter ^ (ObjectKind.POLYGON | ObjectKind.EXTENDED_POLYGON)) != 0) {
      readMap(minLong, maxLong, minLat, maxLat, resolution, (ObjectKind.ALL ^ (ObjectKind.POLYGON | ObjectKind.EXTENDED_POLYGON)) & objectKindFilter, null, listener, files);
    }
  }
