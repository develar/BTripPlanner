...
public class StatsListener implements TransformedMapListener
{
    ...
    public String toString()
    {
        ...
        if(nbPoints>0) result.append("p=").append(nbPoints);
        if(nbPolygons>0) result.append(" pg=").append(nbPolygons);
        if(nbPolygonPoints>0) result.append(" pgp=").append(nbPolygonPoints);
        if(nbPolylines>0) result.append(" ln=").append(nbPolylines);
        if(nbPolylinePoints>0) result.append(" lnp=").append(nbPolylinePoints);
