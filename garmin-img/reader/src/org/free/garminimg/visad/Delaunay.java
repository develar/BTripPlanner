...
// Delaunay.java
//
package org.free.garminimg.visad;
...
public abstract class Delaunay implements java.io.Serializable {
  private static final Logger LOG = Logger.getInstance(Delaunay.class);
 ...
 account unless it is extended as well. */
    ...
    try {
      ...
      if (choice == WATSON) {
        ...
        return new DelaunayWatson(samples);
      }
    }
    catch (Exception e) {
      if (choice != CLARKSON) {
        try {
          // triangulate with the Clarkson method
          return new DelaunayClarkson(samples);
        }
  ...
  public boolean test(float[][] samples, boolean printErrors) {
    ...
    for (int i = 0; i < ntris; i++) {
      if (Tri[i].length < dim1) {
        if (printErrors) {
          LOG.error("Delaunay.test: invalid triangulation " + "dimension (Tri[" + i + "].length=" + Tri[i].length + "; dim1=" + dim1 + ")");
        }
        return false;
      }
    }
    ...
    for (int i = 0; i < ntris; i++) {
      for (int j = 0; j < dim1; j++) {
        if (Tri[i][j] < 0 || Tri[i][j] >= nrs) {
          if (printErrors) {
            LOG.error("Delaunay.test: illegal triangle vertex (Tri[" + i + "][" + j + "]=" + Tri[i][j] + "; nrs=" + nrs + ")");
          }
          return false;
        }
      }
    ...
    for (int i = 0; i < nrs; i++) {
      if (nverts[i] == 0) {
        if (printErrors) {
          LOG.error("Delaunay.test: point not in triangle (" +
                    "nverts[" + i + "]=0)");
        }
        return false;
      }
    ...
    for (int i = 0; i < ntris; i++) {
      for (int j = i + 1; j < ntris; j++) {
        ...
        if (mtot) {
          if (printErrors) {
            LOG.error("Delaunay.test: duplicate triangles (i=" + i + "; j=" + j + ")");
          }
          return false;
        }
      }
    ...
    for (int i = 0; i < ntris; i++) {
      for (int j = 0; j < dim1; j++) {
        if (Walk[i][j] != -1) {
          ...
          if (!found) {
            if (printErrors) {
              LOG.error("Delaunay.test: error in Walk array (i=" + i + "; j=" + j + ")");
            }
            return false;
          }
          ...
          if (sb != dim) {
            if (printErrors) {
              LOG.error("Delaunay.test: error in Walk array (i=" + i + "; j=" + j + "; sb=" + sb + "; dim=" + dim + ")");
            }
            return false;
          }
        }
          ...
          if (edge[curedge]) {
            ...
            if (t2 >= 0) {
              ...
              if (d) {
                ...
                tn1[ln1] = t2;
                int c = 0;
                for (int aVertn2 : vertn2) {
                  if (aVertn2 != t2) {
                    tn2[c++] = aVertn2;
                  }
                }
                c = 0;
                for (int aVertn3 : vertn3) {
                  if (aVertn3 != t) {
                    tn3[c++] = aVertn3;
                  }
                }
    ...
    if (Walk == null && mdim <= 3) {
      ...
      for (int i = 0; i < ntris; i++) {
        ...
        for (int j = 0; j < mdim1; j++) {
          int v2 = (j + 1) % mdim1;
          Walk[i][j] = -1;
          for (int k = 0; k < Vertices[Tri[i][j]].length; k++) {
            int temp = Vertices[Tri[i][j]][k];
            if (temp != i) {
...
*/
  ...
  public String sampleString(float[][] samples) {
    ...
    if (samples != null) {
      s.append("\nsamples ").append(samples[0].length).append("\n");
      for (int i = 0; i < samples[0].length; i++) {
        s.append("  ").append(i).append(" -> ").append(samples[0][i]).append(" ").append(samples[1][i]).append(" ").append(samples[2][i]).append("\n");
      }
      s.append("\n");
    }

    s.append("\nTri (triangles -> vertices) ").append(Tri.length).append("\n");
    for (int i = 0; i < Tri.length; i++) {
      s.append("  ").append(i).append(" -> ");
      for (int j = 0; j < Tri[i].length; j++) {
        s.append(" ").append(Tri[i][j]);
      }
      s.append("\n");
    }

    s.append("\nVertices (vertices -> triangles) ").append(Vertices.length).append("\n");
    for (int i = 0; i < Vertices.length; i++) {
      s.append("  ").append(i).append(" -> ");
      for (int j = 0; j < Vertices[i].length; j++) {
        s.append(" ").append(Vertices[i][j]);
      }
      s.append("\n");
    }

    s.append("\nWalk (triangles -> triangles) ").append(Walk.length).append("\n");
    for (int i = 0; i < Walk.length; i++) {
      s.append("  ").append(i).append(" -> ");
      for (int j = 0; j < Walk[i].length; j++) {
        s.append(" ").append(Walk[i][j]);
      }
      s.append("\n");
    }

    s.append("\nEdges (triangles -> global edges) ").append(Edges.length).append("\n");
    for (int i = 0; i < Edges.length; i++) {
      s.append("  ").append(i).append(" -> ");
      for (int j = 0; j < Edges[i].length; j++) {
        s.append(" ").append(Edges[i][j]);
      }
      s.append("\n");
    }
    return s.toString();
