...
//	private static final String TAG = "oruxmaps -TreSubFile->";
    ...
    private WeakReference<List<SubDivision>> pointerToRootSubDIvisions = new WeakReference<List<SubDivision>>(null);    
    ...
    public TreSubFile(String filename, String filetype, int fileSize, int blocSize, int initialOffset, ImgFileBag fileBag)
    {
    ...
    {
        if(pointerToRootSubDIvisions.get()==null)
        {            
            FileContext context=new FileContext();
            parseLevels(context);
    ...
    private void parseSubDivisions(FileContext context) throws IOException
    {
        ...
        List<SubDivision> rootSubDivisions=new ArrayList<SubDivision>();
        pointerToRootSubDIvisions = new WeakReference<List<SubDivision>>(rootSubDivisions);
        ArrayList<SubDivision> subDivisionsByIndex = new ArrayList<SubDivision>();
        parseSubDivision(subDivisionOffset, subDivisionLength, subDivisionsByIndex, rootSubDivisions, maxLevel, 1, context);//subDivisions
        parseLastSubDivision(subDivisionOffset, subDivisionLength, subDivisionsByIndex, rootSubDivisions, context);//subDivisions
    ...
    private void adjustSubDivisions(ArrayList<SubDivision> subDivisionsByIndex) throws IOException
    {
        for(int cpt=1; cpt<subDivisionsByIndex.size()-1; ++cpt)
        {
            ...
            if (extendedTypes){
            	nextRegion.setDataExtendedPoiOffset(curRegion.getDataExtPoiEnd());
            	nextRegion.setDataExtendedPolygonOffset(curRegion.getDataExtPolygonEnd());
            	nextRegion.setDataExtendedPolylineOffset(curRegion.getDataExtPolylineEnd());
            }
        }
        SubDivision curRegion = subDivisionsByIndex.get(subDivisionsByIndex.size()-1);
        curRegion.setDataEnd(0);
    }
    
    
    private void parseLastSubDivision(long subDivisionOffset, long subDivisionLength, ArrayList<SubDivision> subDivisionsByIndex, List<SubDivision> subDivisions, FileContext context) throws IOException{
    	...
    	for (SubDivision s: subDivisions){
        	...
        	if (nextIndex <= lastBigIndex){        		
        		parseLastSubDivision(subDivisionOffset, subDivisionLength, subDivisionsByIndex, s.getSubDivisions(), context);  		
        	}else{
            	...
            	if (nextlevel == minLevel){
            		...
            		pos=subDivisionOffset+(lastBigIndex)*FULL_DIVISION_RECORD+(nextIndex-lastBigIndex-1)*SMALL_DIVISION_RECORD;
            		...
            		{
            			SubDivision cur=new SubDivision(nextIndex, nextlevel, this);
            			boolean last=cur.parse(this, context, SMALL_DIVISION_RECORD);
            			registerRegionByIndex(subDivisionsByIndex, nextIndex, cur);
            			subs.add(cur);
            			if(cur.getDataOffset()!=0)
            				maxLevelWithData=Math.max(maxLevelWithData, nextlevel);
            			if(last)
            			{
            				break;
            			}
    ...
    private void parseSubDivision(long subDivisionOffset, long subDivisionLength, ArrayList<SubDivision> subDivisionsByIndex, List<SubDivision> subs,
    		int level, int index, FileContext context) throws IOException
    		{
    	...
    	if (nextlevel >0){
    		...
    		{
    			parseSubDivision(subDivisionOffset, subDivisionLength, subDivisionsByIndex, s.getSubDivisions(), nextlevel, s.getIndexNextLevel(), context);
    		}
    	}
    }
    ...
    public void registerRegionByIndex(ArrayList<SubDivision> subDivisionsByIndex, int index, SubDivision division)
    {
    ...
    {
        initIfNeeded();
        List<SubDivision> rootSubDivisions = pointerToRootSubDIvisions.get();
    ...
    public void readMap(int minLong, int maxLong, int minLat, int maxLat, int resolution, int objectKindFilter, BitSet objectTypeFilter, RgnSubFile rgn, LblSubFile lbl, NetSubFile net, MapListener listener) throws IOException
    {
        ...
        {            
            ...
            {
        		s.readMap(minLong, maxLong, minLat, maxLat, level, objectKindFilter, objectTypeFilter, rgn, lbl, net, listener, rgnContext);
            }     
        }
    }
