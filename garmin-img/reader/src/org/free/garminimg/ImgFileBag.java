...
public class ImgFileBag {
  ...
  private ArrayList<ImgSubFile> otherSubFiles = new ArrayList<>(1);
   ...
   */
  private HashMap<Long, WeakReference<byte[]>> blocCache = new HashMap<>();
  ...
  public ImgFileBag(File file, ImgFilesBag parent, boolean xored) throws IOException {
    this.file = file;
    this.parent = parent;
    this.xored = xored;
  }
  ...
  public synchronized byte[] getBloc(long pos, long blocSize) throws IOException {
    ...
    if (reference == null || (result = reference.get()) == null) {
      ...
      blocCache.put(pos, new WeakReference<>(result));
      releaseInput();
    }
    return result;
  }
  ...
  void buildFromSubFiles(LblSubFile lbl, NetSubFile net, RgnSubFile rgn, TreSubFile tre, String description) {
