...
class RgnSubFile extends ImgSubFile {
  private static final Logger LOG = Logger.getInstance(RgnSubFile.class);
...
//    private boolean extendedTypes;
  ...
  private long extendedPolygonsOffset, extendedPolylinesOffset, extendedPoisOffset;

//    private long extendedPolygonsLength, extendedPolylinesLength, extendedPoisLength;
  ...
  public RgnSubFile(String filename, String filetype, int fileSize, int blocSize, int initialOffset, ImgFileBag fileBag) {
...
//        	extendedPoisLength = readUInt32(context);
  ...
  public void parseSubDivision(SubDivision subDivision, LblSubFile lbl, NetSubFile net, MapListener listener,
    ...
    if ((objectKindFilter & ObjectKind.EXTENDED_POLYGON) != 0) {
      ...
      if (segment != null) {
        parseExtPoly(subDivision, net, segment, listener, false, targetLevel, objectTypeFilter, rgnContext);
      }
    }
    ...
    if ((objectKindFilter & ObjectKind.EXTENDED_POLYLINE) != 0) {
      ...
      if (segment != null) {
        parseExtPoly(subDivision, net, segment, listener, true, targetLevel, objectTypeFilter, rgnContext);
      }
    }
    ...
    if ((objectKindFilter & ObjectKind.EXTENDED_POINT) != 0) {
      ...
      if (segment != null) {
        parseExtPoints(subDivision, segment, listener, false, targetLevel, objectTypeFilter, rgnContext.context);
      }
    }
  }
  ...
  private void parsePoly(SubDivision subDivision, NetSubFile net, Segment segment, MapListener listener, boolean line,
    ...
    if (getNextReadPos(context) > segment.segmentEnd) {
      LOG.error("Bad poly* subDivision end: expected=" + segment.segmentEnd + " actual=" + getNextReadPos(context) + " index=" + subDivision.getIndex());
    }
  }
   ...
   */
  private void parseExtPoints(SubDivision subDivision,
                              Segment segment,
                              MapListener listener,
                              boolean indexed,
                              int targetLevel,
                              BitSet objectTypeFilter,
    ...
    while (getNextReadPos(context) < segment.segmentEnd) {
...

      {
        ...
        listener.addPoint(fullTipo >> 8, fullTipo & 0xff, subDivision.getLongitude(longitudeDelta, 0), subDivision.getLatitude(
          latitudeDelta, 0), label, indexed);
      }
    }
    if (getNextReadPos(context) > segment.segmentEnd) {
      LOG.error("Bad segment end: expected=" + segment.segmentEnd + " actual=" + getNextReadPos(context));
    }
  }
   ...
   * Parse all the poly[gons|lines] of the RGN and send the results to the listener.
   */
  private void parseExtPoly(SubDivision subDivision, NetSubFile net, Segment segment, MapListener listener, boolean line,
                            int targetLevel, BitSet objectTypeFilter, RgnContext rgnContext) throws IOException {
    final FileContext context = rgnContext.context;
    ...
    while (getNextReadPos(context) < segment.segmentEnd) {
      ...
      final int bitStreamInfo = readByte(context);
      {
        ...
        final int longBits = convertCoordinateLength(bitStreamInfo & 0xF, longSign, longExtraBit);
        final int latBits = convertCoordinateLength(bitStreamInfo >> 4, latSign, 0);
        ...
        try {
          ...
          while (reader.hasNext(longBits + latBits)) {
            int longOffset = reader.readCoordOffset(longBits, longSign, longExtraBit, context);
            int latOffset = reader.readCoordOffset(latBits, latSign, 0, context);
            curLongPos += longOffset;
            curLatPos += latOffset;
            rgnContext.checkCoordsSize(nbPoints);
            rgnContext.longs[nbPoints] = subDivision.getLongitude(curLongPos, longExtraBit);
          ...
          System.out.println("SubDivision " + segment.segmentEnd + " actual=" + getNextReadPos(context)
        ...
        listener.addPoly(fullTipo, rgnContext.longs, rgnContext.lats, nbPoints, label, line);
      }
    }
    if (getNextReadPos(context) > segment.segmentEnd) {
      LOG.error("Bad poly* subDivision end: expected=" + segment.segmentEnd + " actual=" + getNextReadPos(context) + " index=" + subDivision.getIndex());
  ...
  private static Label translateLabel(int type, Label label, boolean line) {
    if (label != null && line) {
      if (type >= ImgConstants.MINOR_LAND_CONTOUR && type <= ImgConstants.MAJOR_DEPTH_CONTOUR) {   //contour lines are in feet
  ...
  private static Label translateLabel(Label label) {
    double feet = 0.;
    try {
  ...
  private static int convertCoordinateLength(int i, int sign, int extraBit) {
   ...
   * Parse all the points of the RGN and send the results to the listener.
   */
  private void parsePoints(SubDivision subDivision,
                           Segment segment,
                           MapListener listener,
                           boolean indexed,
                           int targetLevel,
  ...
  private Segment[] getSegments(SubDivision subDivision, ImgSubFile.FileContext context) throws IOException {
    ...
    if ((objectTypes & ObjectKind.EXTENDED_POLYGON) != 0) {
      ...
      if (fin > ini) {
        result[SEG_POS_EXTPOLYGON] = new Segment(ObjectKind.EXTENDED_POLYGON);
        result[SEG_POS_EXTPOLYGON].setPosition(extendedPolygonsOffset + ini, extendedPolygonsOffset + fin);
      }
    }
    if ((objectTypes & ObjectKind.EXTENDED_POLYLINE) != 0) {
      ...
      if (fin > ini) {
        result[SEG_POS_EXTPOLYLINE] = new Segment(ObjectKind.EXTENDED_POLYLINE);
        result[SEG_POS_EXTPOLYLINE].setPosition(extendedPolylinesOffset + ini, extendedPolylinesOffset + fin);
      }
    }
    if ((objectTypes & ObjectKind.EXTENDED_POINT) != 0) {
      ...
      if (fin > ini) {
        result[SEG_POS_EXTPOINT] = new Segment(ObjectKind.EXTENDED_POINT);
        result[SEG_POS_EXTPOINT].setPosition(extendedPoisOffset + ini, extendedPoisOffset + fin);
      }
    }
    return result;
  }
