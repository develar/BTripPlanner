...
public class MapPanel<COORD> extends JPanel {
  ...
  public MapPanel(Converter<COORD> converter, int margin) {
    this(new ImgFilesBag(), new MapTransformer<>(converter, margin));
  }
  ...
  public MapPanel(ImgFilesBag map, MapTransformer<COORD> transformer) {
    ...
    thread = new MapPanelThread<>(this, config);
    thread.start();
  ...
  public void addMapLocation(File location) throws IOException {
    if (location.isDirectory()) {
      ...
      map.addFile(location, true, null);
    }
  ...
  public void saveMapAs(Graphics2D g2, MapTransformer<COORD> transformer, float fontSize) throws IOException {
    ...
    TransformedMapListener drawer = createMapDrawer(new MapConfig(), g2, fontSize, transformer);
    MapListener listener = new CoordinateConverterListener<>(transformer,
